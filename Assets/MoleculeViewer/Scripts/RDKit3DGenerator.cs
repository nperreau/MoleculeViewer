...
{
    public enum MoleculeOptimization
    {
        Auto,
        None,
        UFF,
    ...
    public class RDKit3DGenerator : MonoBehaviour
    {
        ...
        public MoleculeOptimization moleculeOptimization = MoleculeOptimization.Auto;

        [SerializeField]
        private GameObject generatingText;
        ...
        private readonly List<AtomRenderData> cachedAtomsData = new();
        ...
        private void Awake() => activeAtomMesh = GetAtomMesh();
        ...
        public readonly struct Expected<TSuccess, TFailure>
        {
            ...
            public Expected(TSuccess successValue)
            {
                Success = true;
            ...
            public Expected(TFailure failureValue)
            {
                Success = false;
            ...
            public TSuccess GetSuccessValue()
            {
                if (!Success)
                    throw new InvalidOperationException("Trying to get Success value but the expected failed.");

                return _successValue;
            }

            public TFailure GetFailureValue()
            {
                if (Success)
                    throw new InvalidOperationException("Trying to get Failure value but the expected succeeded.");

                return _failureValue;
            }
        }
        
        public async Task GenerateAndPrepareMolecule(string smilesString)
        {
            ...
            try
            {
                ...
                var result = await LaunchRDKitEmbedingThread(mol, moleculeOptimization);

                if (!result.Success)
                {
                    Debug.Log($"Failed to generate embedding: {result.GetFailureValue()}");
                    HideGenerationLoading();
                    return;
                }

                BuildRenderGroups(result.GetSuccessValue());
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    ...
                    if (embedResult < 0)
                        ...
                        OptimizeMolecule(mol, moleculeOptimization);
                });
            }
            catch (Exception e)
            {
                mol.Dispose();
                return new Expected<RWMol, Exception>(e);
            }

            return new Expected<RWMol, Exception>(mol);
        }

        private static void OptimizeMolecule(ROMol mol, MoleculeOptimization moleculeOptimization)
        {
            try
            {
                switch (moleculeOptimization)
                {
                    case MoleculeOptimization.None:
                        break;
                    case MoleculeOptimization.UFF:
                        if (ForceField.UFFHasAllMoleculeParams(mol))
                            ForceField.UFFOptimizeMolecule(mol);
                        break;
                    case MoleculeOptimization.MMFF:
                        if (ForceField.MMFFHasAllMoleculeParams(mol))
                            ForceField.MMFFOptimizeMolecule(mol);
                        break;
                    case MoleculeOptimization.Auto:
                        if (ForceField.UFFHasAllMoleculeParams(mol))
                            ForceField.UFFOptimizeMolecule(mol);
                        else if (ForceField.MMFFHasAllMoleculeParams(mol))
                            ForceField.MMFFOptimizeMolecule(mol);
                        else
                            moleculeOptimization = MoleculeOptimization.None;
                        break;
                    default:
                        throw new ArgumentOutOfRangeException(nameof(moleculeOptimization), moleculeOptimization, null);
                }

            }
            catch (Exception e)
            {
                Debug.LogWarning($"Molecule optimization skipped: {e.Message}");
            }
        }

        private void BuildRenderGroups(ROMol mol)
        {
            cachedAtomsData.Clear();
            ...
            using (Conformer conformer = mol.getConformer())
            {
                ...
                for (uint i = 0; i < atomCount; i++)
                {
                    ...
                    cachedAtomsData.Add(new AtomRenderData(symbol, position));
                }
            }
        ...
        private void ApplyVisualTransforms()
        {
            ...
            if (cachedAtomsData.Count == 0)
                return;

            foreach (AtomRenderData atom in cachedAtomsData)
            {
                float scaleFactor = GetAtomScale(atom.Symbol) * atomScale;
                Vector3 position = atom.Position * coordinateScale;
        ...
        {
            if (Mathf.Approximately(atomScale, newScale)) 
                return;
